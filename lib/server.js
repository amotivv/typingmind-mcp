...
async function startClient(clientId, config) {
  ...
  const transport = new StdioClientTransport({
    command, // We don't sanitize the command itself as it should point to a valid executable
    args: sanitizedArgs,
    env:
      Object.values(sanitizedEnv).length > 0
        ? {
  ...
  // Store the client with its ID and sanitized values
  clients.set(clientId, {
    ...
    config: {
      ...
      args: sanitizedArgs,
      env: sanitizedEnv
    }, // Store sanitized config for restart
    createdAt: new Date(),
  });
...
async function start(authToken) {
  ...
  app.post('/start', auth, validateStartRequest, async (req, res) => {
    try {
  ...
  app.post('/restart/:id', auth, validateClientId, async (req, res) => {
    const { id } = req.params;
  ...
  app.get('/clients/:id', auth, validateClientId, (req, res) => {
  ...
  app.get('/clients/:id/tools', auth, validateClientId, async (req, res) => {
    const { id } = req.params;
    const clientEntry = clients.get(id);
  ...
  app.post('/clients/:id/call_tools', auth, validateClientId, validateCallTools, async (req, res) => {
    const { id } = req.params;
  ...
  app.delete('/clients/:id', auth, validateClientId, async (req, res) => {
    const { id } = req.params;
    const clientEntry = clients.get(id);
  ...
  // Global error handler - sanitizes error responses
